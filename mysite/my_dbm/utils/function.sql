...
BEGIN
    ...
    SELECT NOW(), NOW(), TRUE,
           ltd.col_date_create, ltd.col_type, ltd.col_columns, ltd.col_is_null, ltd.col_is_key,
           ltd.col_unique_together, ltd.col_default, ltd.col_description, lts.id
    FROM my_dbmatch.link_total_data AS ltd
        LEFT JOIN my_dbmatch.dim_stage         AS dst ON dst.name = ltd.stage
        LEFT JOIN my_dbmatch.link_db           AS ldb ON ldb.stage_id = dst.id AND ldb.alias = ltd.db_name
        LEFT JOIN my_dbmatch.link_base_schemas AS lbs ON lbs.base_id = ldb.base_id AND lbs.schema = ltd.schem_name
    ...
    SET is_active = TRUE
    WHERE id IN (SELECT column_id FROM my_dbmatch.link_columns_stage WHERE is_active);

    UPDATE my_dbmatch.link_tables
    SET is_active = TRUE
    WHERE id IN (SELECT DISTINCT table_id FROM my_dbmatch.link_columns WHERE is_active);

    UPDATE my_dbmatch.link_base_schemas
    SET is_active = TRUE
    WHERE id IN (SELECT DISTINCT schema_id FROM my_dbmatch.link_tables WHERE is_active);
