...
# === Вспомогательные API (APIView) ===

@extend_schema(
    tags=['app_dbm'],
    summary="Получить схемы по ID базы данных",
    parameters=[
        OpenApiParameter(name='db_id', type=int, required=True, description="ID записи в DimDB")
    ],
    responses={200: LinkSchemaSerializer(many=True)}
)
class SchemaByDBAPI(APIView):
    ...
    def get(self, request):
        ...
        if not db_id:
            return Response(
                {'error': 'Параметр db_id обязателен'},
                status=status.HTTP_400_BAD_REQUEST
            )
        schemas = get_list_or_404(LinkSchema, base_id=db_id)
        serializer = LinkSchemaSerializer(schemas, many=True)
        return Response(serializer.data)


@extend_schema(
    tags=['app_dbm'],
    summary="Получить таблицы по ID схемы",
    parameters=[
        OpenApiParameter(name='schema_id', type=int, required=True, description="ID записи в LinkSchema")
    ],
    responses={200: LinkTableSerializer(many=True)}
)
class TableBySchemaAPI(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        if not schema_id:
            return Response(
                {'error': 'Параметр schema_id обязателен'},
                status=status.HTTP_400_BAD_REQUEST
            )
        tables = get_list_or_404(LinkTable, schema_id=schema_id)
        serializer = LinkTableSerializer(tables, many=True)
        return Response(serializer.data)


@extend_schema(
    tags=['app_dbm'],
    summary="Получить колонки по ID таблицы",
    parameters=[
        OpenApiParameter(name='table_id', type=int, required=True, description="ID записи в LinkTable")
    ],
    responses={200: LinkColumnSerializer(many=True)}
)
class ColumnByTableAPI(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        if not table_id:
            return Response(
                {'error': 'Параметр table_id обязателен'},
                status=status.HTTP_400_BAD_REQUEST
            )
        columns = get_list_or_404(LinkColumn, table_id=table_id)
