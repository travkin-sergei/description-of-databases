...
from app_dbm.models import DimDB, LinkSchema, LinkTable, LinkColumn, DimTypeLink, LinkColumnColumn
...
class DimUpdateMethodChangeForm(forms.ModelForm):
    ...
    main_schema = forms.ModelChoiceField(queryset=LinkSchema.objects.none(), label="Схема (основной)", required=True)
    main_table = forms.ModelChoiceField(queryset=LinkTable.objects.none(), label="Таблица (основной)", required=True)
    ...
    sub_schema = forms.ModelChoiceField(queryset=LinkSchema.objects.none(), label="Схема (связанный)", required=False)
    sub_table = forms.ModelChoiceField(queryset=LinkTable.objects.none(), label="Таблица (связанный)", required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'main_base' in self.data:
            try:
                ...
                self.fields['main_schema'].queryset = LinkSchema.objects.filter(base_id=base_id)
            except (ValueError, TypeError):
                self.fields['main_schema'].queryset = LinkSchema.objects.none()
        else:
            self.fields['main_schema'].queryset = LinkSchema.objects.none()
        ...
        if 'main_schema' in self.data:
            try:
                ...
                self.fields['main_table'].queryset = LinkTable.objects.filter(schema_id=schema_id)
            except (ValueError, TypeError):
                self.fields['main_table'].queryset = LinkTable.objects.none()
        else:
            self.fields['main_table'].queryset = LinkTable.objects.none()
        ...
        if 'sub_base' in self.data:
            try:
                ...
                self.fields['sub_schema'].queryset = LinkSchema.objects.filter(base_id=base_id)
            except (ValueError, TypeError):
                self.fields['sub_schema'].queryset = LinkSchema.objects.none()
        else:
            self.fields['sub_schema'].queryset = LinkSchema.objects.none()
        ...
        if 'sub_schema' in self.data:
            try:
                ...
                self.fields['sub_table'].queryset = LinkTable.objects.filter(schema_id=schema_id)
            except (ValueError, TypeError):
                self.fields['sub_table'].queryset = LinkTable.objects.none()
        else:
            self.fields['sub_table'].queryset = LinkTable.objects.none()
